...
在mxGraph中，分组的概念就是逻辑上将一个图元与另一个进行关联。在许多的图形开发库中，这通常被成为子图。在图形模型的数据结构中，分组就是把一个或更多的连线或顶点变成一个父顶点或连线的子图元。分组使得mxGraph拥有了更多的有用的特性：

- 子图 一个逻辑上独立的图形的概念，在较∏高的层次图中显示为每个子图的图元。
- 展开和折叠 折叠是这样的一种能力，使用父图元来替换一组子图元的显示。 在[online workfloweditor](http://www.jgraph.com/demo/mxgraph/editors/workfloweditor.html)（在线工作流编辑器）示例中，在泳道的组单元例子里，点击左上角的小“-”就可以看到效果。这在下面的复杂度管理的章节有描述。
- 分层 层次的概念是指在图形的显示中，让图元以特定的z轴顺序显示。
- 下钻，递升 这些概念是让子图可以当作完整的图形一样可视化和编辑。在用户对象的章节，我们看到了 “检查库存”节点作为一个图元。举个例子来说，一个开发员在描述的是每一个节点是这个过程中的作为执行任务的软件进程。应用程序可能有一个选项，可以下钻到检查库存节点。这将导致出现一个新的图形来详细描述了系统究竟是如何检查库存。 该图可能有标题“检查库存”的父节点，以表明它是一个子图，以及一个选择可以退回到下个更高级别。

在分组中，图元都分配了一个父图元。在最简单的情况下，所有的图元具有默认的父单元为他们的父图元。默认父图元是一种与图形具有相同的界限的无形的图元。在HelloWorld例子中，这是由graph.getDefaultParent()返回的图元。一个节点的x，y 坐标是其相对于其父图元的位置，这样的情况下，默认分组（所有图元共享的缺省父图元）的图元定位也对于图形组件的绝对坐标。在所有图元被添加到默认的根的情况下，如HelloWorld例子，组结构在逻辑上看起来如下图所示。

请注意增加的第0层图元，这是组结构中的默认间接方式，允许层更改需要额外的单元格。为正确性，我们包括它下面部分， 但在以后的组图中，它将被省略。

![image](https://jgraph.github.io/mxgraph/docs/images/mx_man_hello_struct.png)

_helloworld示例中的组结构_

同时，请注意连线标签的位置（几何上的x,y）是相对于其父图元的。

如果我们回头去看用户对象章节的simple workflow（简单工作流）这个例子，我们可以看到组看起来很直观。在这个例子里，组图元代表人，子节点代表分配给这些人的任务。在这个例子里，组的逻辑结构如下：
![https://jgraph.github.io/mxgraph/docs/images/mx_man_log_group_struct.png](https://jgraph.github.io/mxgraph/docs/images/mx_man_log_group_struct.png)

_工作流例子中组的逻辑结构_

工作流操作节点是黄色的子节点而泳道组节点被标记成了蓝色。

插入图元到已经存在的组结构中，需要使用mxGraph类上的insertVertex和insertEdge两个方法并将组作为parent参数传入。这些方法通过设置对子图元相应的父图元，而且重要的是，要通知父图元子图元的加入。

修改组结构是通过mxGraph.groupCells()和mxGraph.ungroupCells()的方法来实现的。

**核心API方法：**

* **mxGraph.groupCells(group, border, cells)** – 在开始/结束更新之间，把指定的图元加入到指定的组

* **mxGraph.ungroupCells(cells)** – 把特定的图元从它的父图元中移除， 并把它们加入到它们父图元的父图元。操作之后的空组会被删除。这些操作都需要包裹在开始/结束更新之间。

### 3.1.5 复杂度管理

控制任何一次显示的图元数量有两个主要原因。首先是性能，在任何平台上绘制越来越多的图元总会在某一时刻达到性能的可用性限制。第二个原因是易用性，人只能理解一定量的信息。与上面列出的分组相关联的所有概念可以降低用户屏幕砂锅信息的复杂性。

#### 3.1.5.1 可折叠

可折叠是我们用来描述组的展开和折叠一个集合术语。如果我们使一个图元的子节点不可见，便可以说这个图元被折叠了。这里有几个方法和这个特性相关：

**核心API方法**

* **mxGraph.foldCells(collapse, recurse, cells)** – 说明指定单元格的折叠状态

**可折叠相关的方法：**

* **mxGraph.isCellFoldable(cell, collapse)** – 对有子图元的图元而言，默认为真
...
图元的子图元变为不可见。
组图元的组边界会被使用。在mxGeometry中有一个alternativeBounds字段，在组图元中，默认为其折叠和展开状态存储一个单独的边界值。